...
from src.job.douyin.schemas import FetchSelectedUserVideosRequest
...
@router.post("/users/fetch-videos")
async def create_fetch_selected_user_videos_job(
    request: Request,
    ...
    """Create a job that fetches videos for selected TikHub users."""
    job = job_service.create_job()
    background_tasks.add_task(
        douyin_job_service.run_fetch_selected_user_videos_job,
        job.id,
        payload.user_ids,
        db,
        tikhub,
